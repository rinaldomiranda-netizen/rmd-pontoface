...
function EmployeePunch() {
  ...
  async function getGeolocation(): Promise<{ latitude: number | null; longitude: number | null; accuracy: number | null }> {
    ...
    return new Promise(resolve => {
      let best: { latitude: number | null; longitude: number | null; accuracy: number | null } = { latitude: null, longitude: null, accuracy: null };
      let finished = false;
      ...
      watchId = navigator.geolocation.watchPosition(
        p => {
          const item = { latitude: p.coords.latitude, longitude: p.coords.longitude, accuracy: p.coords.accuracy };
          if (best.accuracy == null || item.accuracy == null || item.accuracy < best.accuracy) best = item;
          if (item.accuracy != null && item.accuracy <= 25) finish();
        },
        () => finish(),
