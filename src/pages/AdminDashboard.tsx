...
export default function AdminDashboard({ companyId, role, onLogout }: Props) {
  ...
  const [labels, setLabels] = React.useState({ person_label: 'Funcionário', people_label: 'Funcionários', entry_label: 'Bater entrada', exit_label: 'Bater saída', exit_enabled: true, presence_mode: 'both', name: '', logo_url: null as string | null });
  ...
  React.useEffect(() => {
    supabase.from('companies').select('name,logo_url,person_label,people_label,entry_label,exit_label,exit_enabled,presence_mode').eq('id', companyId).maybeSingle()
      .then(({ data }) => { if (data) setLabels(prev => ({ ...prev, ...(data as any) })); setLabelsLoaded(true); });
  }, [companyId]);
  ...
  async function refreshLiveCompany() {
    try {
      ...
      if (companyData) setLabels(prev => ({ ...prev, ...(companyData as any) }));
  ...
  React.useEffect(() => {
    const onUpdated = (e: Event) => {
      ...
      setLabels(prev => ({ ...prev, ...d }));
    };
    window.addEventListener('rmd:company-updated', onUpdated);
    return () => window.removeEventListener('rmd:company-updated', onUpdated);
  }, []);

  const companyName = (labels.name || '').trim();
  React.useEffect(() => {
    document.title = companyName ? `${companyName} · PontoFace` : 'PontoFace';
  }, [companyName]);
  const today = new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' });
  const roleLabel = role === 'owner' ? 'Proprietário' : role === 'admin' ? 'Administrador' : role === 'hr' ? 'RH' : role === 'viewer' ? 'Consulta' : 'Equipe';
  ...
  return (
    ...
    <div className="admin theme-company">
      <div className="admin-shell">
        <nav className="admin-nav">
          <div className="nav-company">
            <CompanyMark name={companyName} logoUrl={labels.logo_url} size={40} />
            <div className="nav-company-text">
              <div className="nav-company-name">{companyName || (labelsLoaded ? 'Minha empresa' : '')}</div>
              <div className="nav-company-product">PontoFace <span>by RMD</span></div>
            </div>
          </div>
          {nav}
        ...
        <main className="admin-main">
          <div className="admin-topbar">
            {section !== 'overview' && <button className="btn light back-btn" onClick={() => setSection('overview')} aria-label="Voltar ao dashboard">←</button>}
            <div className="company-header">
              <CompanyMark name={companyName} logoUrl={labels.logo_url} size={52} />
              <div className="company-header-text">
                <div className="company-header-name">{companyName || (labelsLoaded ? 'Minha empresa' : ' ')}</div>
                <div className="company-header-section">
                  <b>{navLabel(section)}</b>
                  <span className="company-header-meta">{today} · {roleLabel}</span>
                </div>
              </div>
            </div>
          ...
          {section === 'overview' ? (
            <div className="mobile-nav">
              ...
              {mobileMenuOpen && <div className="mobile-menu-popover" onClick={() => setMobileMenuOpen(false)}>{nav}<button className="mobile-menu-logout" onClick={onLogout}>Sair</button></div>}
            </div>
          ) : (
...
function initialsOf(name: string) {
  const parts = (name || '').trim().split(/\s+/).filter(w => w.length > 2 || /^[A-ZÀ-Ú0-9]/.test(w));
  const letters = (parts.length ? parts : [name || '']).slice(0, 2).map(w => w.charAt(0)).join('');
  return (letters || 'PF').toUpperCase();
}

function CompanyMark({ name, logoUrl, size = 48 }: { name: string; logoUrl?: string | null; size?: number }) {
  const [broken, setBroken] = React.useState(false);
  React.useEffect(() => { setBroken(false); }, [logoUrl]);
  const style = { width: size, height: size, borderRadius: Math.round(size * 0.28) } as React.CSSProperties;
  if (logoUrl && !broken) {
    return <span className="company-mark has-logo" style={style}><img src={logoUrl} alt={name ? `Logotipo ${name}` : 'Logotipo'} onError={() => setBroken(true)} /></span>;
  }
  return <span className="company-mark" style={{ ...style, fontSize: Math.round(size * 0.38) }} aria-hidden="true">{initialsOf(name)}</span>;
}
...
function CompanySettings({ companyId, role }: { companyId: string; role: string }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    if (res.ok) {
      setSaved(true); showToast('Configurações salvas.'); setTimeout(() => setSaved(false), 2000);
      window.dispatchEvent(new CustomEvent('rmd:company-updated', { detail: { name: company.name, logo_url: company.logo_url || null, person_label: company.person_label, people_label: company.people_label } }));
    }
    else alert(friendlyError((res.data as any)?.error));
  }
  async function uploadLogo(file: File | undefined) {
    if (!file) return;
    if (!/^image\/(png|jpeg|webp|svg\+xml)$/.test(file.type)) { alert('Envie uma imagem PNG, JPG, WEBP ou SVG.'); return; }
    if (file.size > 1024 * 1024) { alert('A imagem deve ter no máximo 1 MB.'); return; }
    ...
    if (error) { alert('Não foi possível enviar o logotipo: ' + error.message); return; }
    const { data } = supabase.storage.from('company-logos').getPublicUrl(path);
    setCompany((c: any) => ({ ...c, logo_url: data.publicUrl }));
    showToast('Logotipo carregado. Clique em Salvar para aplicar.');
  }
  ...
  return (
    <div className="card" style={{ maxWidth: 520 }}>
      <form onSubmit={save}>
        ...
        <p className="helptext" style={{ marginBottom: 12 }}>O nome e o logotipo aparecem no cabeçalho do painel e no menu.</p>
        <div className="logo-field">
          <CompanyMark name={company.name || ''} logoUrl={company.logo_url} size={64} />
          <div className="logo-field-actions">
            <label className={`btn light${!canEdit || uploadingLogo ? ' disabled' : ''}`}>
              {uploadingLogo ? 'Enviando...' : company.logo_url ? 'Trocar logotipo' : 'Enviar logotipo'}
              <input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden disabled={!canEdit || uploadingLogo} onChange={e => { void uploadLogo(e.target.files?.[0]); e.target.value = ''; }} />
            </label>
            {company.logo_url && canEdit && <button type="button" className="btn light" onClick={() => setCompany({ ...company, logo_url: null })}>Remover</button>}
            <small className="helptext">PNG, JPG, WEBP ou SVG, até 1 MB. Sem logotipo, aparecem as iniciais.</small>
          </div>
        </div>
